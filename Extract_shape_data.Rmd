...
techdata <- bind_rows(triplex, eidsiva, eco) #
...
remove <- nearZeroVar(x[,-1])+1
techdata <-  techdata[,-remove]
cortest <-cor(techdata[,-1])
hc = findCorrelation(cortest, cutoff=0.9)+1
techdata <- techdata[,-hc]
```
...
```{r}
company2= "Eidsiva"
eidscostfile <-"Eidsiva cost 2.xlsx" #"Kostnadsanalyser Statkraft, Eidsiva-data 2009-2014 (FBi_06-11-2015).xlsx"
# "Eidsiva cost2.xlsx" 

costjoiner2 <- function(company, costfile){
  cost <-read.xlsx(costfile)[,c(4,7:12)]
...
costdata <- bind_rows(eidscost,ecocost, statkraftcost) %>% gather(key = year, value= cost,-Station.ID) %>% group_by(Station.ID) %>% summarise(mean_3yr_2014 = mean(cost)) %>%filter(!is.na(Station.ID),!is.na(mean_3yr_2014))
...
```{r}
data <- inner_join(x, costdata, "Station.ID")

x <-train(mean_3yr_2014 ~ ., data[,-1], method = "lm")
x2 <-train(mean_3yr_2014 ~ ., data[,-1], method = "lmStepAIC")
x3 <- train(mean_3yr_2014 ~,data = data[,-1], method="lm")
test <- select(data, contains("a"))
```
