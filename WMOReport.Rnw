...
Modelling was performed mostly using the caret package in r \cite{Kuhn2015} and following the concepts described by Kuhn and Johnson in their book "Applied Predictive Modelling"\cite{Kuhn2013}. The linear models were used due to their ease of interpretation there were four such models fitted, In addition two nonlinear methods were also evaluated, these models can be harder to interpret but can also give very strong performance if the data is nonlinear.
  \begin{itemize}
